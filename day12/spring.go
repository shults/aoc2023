...
func parseInput(line string) (string, []int) {
	...
	return symbols, brokenLines
}

func CalculateArrangements(line string) int {
	return calculateArrangementsOptimal(parseInput(line))
}
...
func calculateArrangementsPart2(str string, times int) int {
	...
	fmt.Printf("[%s] %v => ", pattern, matchLine)

	res := calculateArrangementsOptimal(pattern, matchLine)
...
func calculateArrangementsSplit(
	pattern string,
	matchLine []int,
) int {
	matchesNum := 0
